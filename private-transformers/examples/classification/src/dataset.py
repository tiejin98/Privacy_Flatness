...
class FewShotDataset_Half(torch.utils.data.Dataset):
    """Few-shot dataset."""
    ...
    def __init__(self, args, tokenizer, cache_dir=None, mode="train", use_demo=False):
        ...
        # Prepare examples (especially for using demonstrations)
        support_indices = list(range(len(self.support_examples)))  # [0, ..., len(train)].
        train_indices = np.random.choice(support_indices,size=int(len(self.support_examples)/2),replace=False)
        out_indices = set(support_indices) - set(train_indices)
        support_indices = train_indices
        np.save("train_index.npy",train_indices)
        np.save("out_index.npy",out_indices)
        self.query_examples = [self.query_examples[inx] for inx in train_indices]
        self.support_examples = [self.support_examples[inx] for inx in train_indices]
        #self.query_emb = [self.query_emb[inx] for inx in train_indices]
        #self.support_emb = [self.support_emb[inx] for inx in train_indices]
