...
from .src.trainer import Trainer,Trainer_prefix,Trainer_awp
from .src.awp import AdvWeightPerturb
...
def main():
    ...
    elif data_args.task_name == "trec":
        data_args.mapping = "{0:'Description',1:'Entity',2:'Expression',3:'Human',4:'Location',5:'Number'}"  
    else:
    ...
    peft_config = PrefixTuningConfig(task_type="SEQ_CLS", inference_mode=False, num_virtual_tokens=50)
    from peft.mapping import get_peft_model_sparse
    model = get_peft_model_sparse(model,peft_config)
    if data_args.task_name == "sst-2":
        sparse_list = [11,5,10]
    elif data_args.task_name == "qnli" or data_args.task_name == "qqp":
        sparse_list = [11,10,9]
    else:
        sparse_list = [1,4,9]
    model.set_sparse_list(sparse_list)
    ...
    emb = torch.load("test_embedding_non_private_{}_sparse.pt".format(data_args.task_name)).to("cuda")
    awp_adv = AdvWeightPerturb(model,device=training_args.device,gamma=1e-4)
    trainer = Trainer_awp(
        model=model,
        awp=awp_adv,
        args=training_args,
    ...
    if training_args.do_train:
        ...
        # Don't reload.
        trainer.train(emb=emb,lamb=0.01,model_path=None)
        # Use the early stop, so do not save the model in the end (unless specify save_at_last)
        eval_dataset = (
        FewShotDataset(data_args, tokenizer=tokenizer, mode="train", use_demo=use_demo)
        if training_args.do_eval else None
            )
        eval_dataloader = trainer.get_eval_dataloader(eval_dataset)
        trainer.prediction_loop_indi(eval_dataloader,description="{}_train_eps{}_final".format(data_args.task_name,privacy_args.target_epsilon))
        ...
        eval_dataloader = trainer.get_eval_dataloader(eval_dataset)
        trainer.prediction_loop_indi(eval_dataloader,description="{}_eval_eps{}_final".format(data_args.task_name,privacy_args.target_epsilon))
