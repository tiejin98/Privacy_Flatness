...
class PrivacyEngine(object):
    ...
    @torch.no_grad()
    def step_prefix_blackbox(self,gradient,alpha):
        """
        Get the norm of gradient first
        """
        current_norm = torch.norm(gradient)
        max_alpha = self.max_grad_norm / (current_norm + 1e-6)
        new_alpha = torch.clamp(alpha,min=-max_alpha,max=max_alpha)
