...
class BertForPromptFinetuning(BertForMaskedLM):
    ...
    def forward(self, input_ids=None, attention_mask=None, token_type_ids=None, mask_pos=None, labels=None,
        ...
        # Encode everything
        outputs = self.bert(input_ids, attention_mask=attention_mask, token_type_ids=token_type_ids,inputs_embeds=inputs_embeds,past_key_values=past_key_values)
...
class RobertaForPromptFinetuning(BertPreTrainedModel):
    ...
    def forward(
        ...
        # Encode everything
        outputs = self.roberta(input_ids, attention_mask=attention_mask,past_key_values=past_key_values,
                               inputs_embeds=inputs_embeds)
...
class RobertaForPromptFinetuning_Low(BertPreTrainedModel):
    ...
    def forward(
        ...
        if self.Q is not None:
            sequence_mask_output = torch.mm(sequence_mask_output,self.Q)
        # # Get <mask> token representation
        # sequence_output, pooled_output = outputs[:2]
        # sequence_mask_output = sequence_output[torch.arange(sequence_output.size(0)), mask_pos]
