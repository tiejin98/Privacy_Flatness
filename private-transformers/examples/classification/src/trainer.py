...
class Trainer(transformers.Trainer):
    ...
    def __init__(self, name=None,model_args=None, privacy_args=None, auxiliary_args=None, **kwargs):
    ...
    def train(self, model_path=None, dev_objective=None, dev_objective_key=None):
        ...
        train_iterator = trange(epochs_trained, int(num_train_epochs), desc="Epoch", disable=False)
    ...
    def train_kd(self, model_path=None, dev_objective=None, dev_objective_key=None,kd=None):
        """
        Main training entry point.
        ...
        for epoch in train_iterator:
            ...
            for step, inputs in enumerate(epoch_iterator):
                ...
                if steps_trained_in_current_epoch > 0:
                    ...
                    continue
                try:
                    kd_batch = kd[real_index*self.args.train_batch_size:
                                  (real_index+1)*self.args.train_batch_size]
                except:
                    kd_batch = kd[real_index*self.args.train_batch_size:]
                kd_batch = torch.from_numpy(kd_batch).to(self.args.device)
                losses = self.training_step_kd(model, inputs,kd_batch)
                tr_loss += losses["scalar_loss"]
                real_index += 1
    ...
    def compute_loss_kd(self, model, inputs, kd,return_outputs=False, return_vector_loss=False):
        """
        How the loss is computed by Trainer. By default, all models return the loss in the first element.
        ...
        loss = F.mse_loss(logits, kd, reduction="none")  # (batch_size,).
    ...
    def training_step_kd(self, model: nn.Module, inputs: Dict[str, Union[torch.Tensor, Any]],kd:Dict) -> dict:
        model.train()
        inputs = self._prepare_inputs(inputs)
        loss = self.compute_loss_kd(model, inputs,kd, return_vector_loss=True)  # (batch_size,).
    ...
    logits)
    """
    def prediction_loop_indi(self, dataloader, description: str, prediction_loss_only: Optional[bool] = None, ignore_keys: Optional[List[str]] = None):

        all_losses = []

        for step, inputs in enumerate(dataloader):
            loss, logits, labels = self.prediction_step(self.model, inputs, prediction_loss_only, ignore_keys=ignore_keys)

            # The following few lines are new, to handle and store individual losses


            loss_fct = nn.CrossEntropyLoss(reduction='none')
            individual_losses = loss_fct(logits.view(-1, self.model.num_labels), labels.view(-1))
            all_losses.append(individual_losses.detach().cpu().numpy())
            # Original code from prediction_loop
            # ...

        # Convert all_losses to a single numpy array and return it
        final_res = np.concatenate(all_losses, axis=0)
        np.save("loss_res_{}.npy".format(description),final_res)
        return final_res

    def prediction_loop_logi(self, dataloader, description: str, prediction_loss_only: Optional[bool] = None, ignore_keys: Optional[List[str]] = None):
        ...
        # Convert all_losses to a single numpy array and return it
        final_res = np.concatenate(all_logits, axis=0)
        np.save("logit_res_{}.npy".format(self.name),final_res)
        return final_res
    ...
    def evaluate(self, eval_dataset: Optional[Dataset] = None) -> Dict[str, float]:
        ...
        #every_loss = self.prediction_loop_indi(eval_dataloader,description="Evaluation")
        self.log(output.metrics)
        #print(np.mean(every_loss))
...
class Trainer_prefix(transformers.Trainer):
    """
    Adding some functions based on Transformers' Trainer class.
    """

    def __init__(self, name=None,model_args=None, privacy_args=None, auxiliary_args=None, **kwargs):
        super(Trainer_prefix, self).__init__(**kwargs)
        self.privacy_args = privacy_args
        self.model_args = model_args
        self.auxiliary_args = auxiliary_args
        self.scaler = torch.cuda.amp.GradScaler(init_scale=128)
        self.name = name
        self.noise = torch.randn_like(self.model.prompt_encoder.default.embedding.weight).to(self.model.prompt_encoder.default.embedding.weight.device)
    ...
    def train_reg(self, emb,lamb,model_path=None, dev_objective=None, dev_objective_key=None):
        """
        Main training entry point.
        ...
        for epoch in train_iterator:
            ...
            for step, inputs in enumerate(epoch_iterator):
                ...
                losses = self.training_step_reg(model,emb,lamb,inputs)
                tr_loss += losses["scalar_loss"]
    ...
    def train_low(self, low_rank,pos_mask_list,lamb,model_path=None, dev_objective=None, dev_objective_key=None):
        """
        Main training entry point.
        ...
        for epoch in train_iterator:
            ...
            if transformers.is_torch_tpu_available():
                ...
                epoch_iterator = tqdm(train_dataloader, desc="Iteration", disable=False)
            mask_pos_list =deepcopy(pos_mask_list)
            for step, inputs in enumerate(epoch_iterator):
                ...
                losses = self.training_step_lowrank(model,low_rank,mask_pos_list,lamb,inputs)
                tr_loss += losses["scalar_loss"]
    ...
    def get_noise(self,model,dataloader):
        for inputs in dataloader:
            loss = self.compute_loss_flat(model,inputs)
            loss.backward()
            break
        for param in model.parameters():
            if param.requires_grad:
                self.noise = param.grad / torch.norm(param.grad)

    def compute_flat(self,model,dataloader):
        self.get_noise(model,dataloader)
        alpha_list = np.linspace(0,1,num=11)
        all_res = []
        for inputs in tqdm(dataloader):
            res = []
            for alpha in alpha_list:
                with torch.no_grad():
                    self.model.prompt_encoder.default.embedding.weight += alpha * self.noise
                    loss = self.compute_loss_flat(model,inputs)
                    res.append(loss.detach().cpu())
                    self.model.prompt_encoder.default.embedding.weight -= alpha * self.noise
            res = np.array(res)
            all_res.append(res)
        all_res = np.array(all_res)
        all_res = np.mean(all_res,axis=0)
        np.save("flat_res_{}.npy".format(self.name),all_res)
        return all_res

    def compute_flat_noise(self,model,dataloader,name):
        alpha_list = np.linspace(-1,1,num=21)
        all_res = []
        for inputs in tqdm(dataloader):
            res = []
            for alpha in alpha_list:
        ...
        all_res = np.array(all_res)
        all_res = np.mean(all_res,axis=0)
        np.save("flat_res_{}.npy".format(name),all_res)
        return all_res



    def compute_loss_flat(self, model, inputs, return_outputs=False, return_vector_loss=False):
        """
        How the loss is computed by Trainer. By default, all models return the loss in the first element.
        ...
        for key in input.keys():
            input[key] = input[key].to("cuda")
        outputs = model(**input)  # This should not contain `loss`.
        logits = outputs[0]
        if isinstance(outputs, SequenceClassifierOutput):
            outputs = (logits,)
        loss = F.cross_entropy(logits, labels, reduction="none")  # (batch_size,).
    ...
    def compute_loss_reg(self, model, emb,lamb,inputs, return_outputs=False, return_vector_loss=False):
        """
        How the loss is computed by Trainer. By default, all models return the loss in the first element.
        ...
        if isinstance(outputs, SequenceClassifierOutput):
            outputs = (logits,)
        loss = F.cross_entropy(logits, labels, reduction="none")  # (batch_size,).
        reg_term = model.prompt_encoder["default"].embedding.weight
        reg_loss = lamb * torch.norm(reg_term - emb)
        loss += reg_loss
        if not return_vector_loss:
            loss = loss.mean(dim=0)
        return (loss, (loss,) + outputs) if return_outputs else loss

    def compute_loss_lowrank(self, model, lowrank,pos_mask_list,lamb,inputs, return_outputs=False, return_vector_loss=False):
        """
        How the loss is computed by Trainer. By default, all models return the loss in the first element.
        ...
        """
        labels = inputs.pop("labels")
        name_list = lowrank.keys()
        activations = {}
        hook_handles = {}
        def get_activation(mask_pos_list, layer_name):
            def hook(model, input, output):
                # Extract the batch index for the current hook call
                batch_index = mask_pos_list['batch_counter']
                # Get the relevant mask position for this batch
                real_index = batch_index // len(name_list)
                try:
                    mask_pos = mask_pos_list['positions'][real_index * self.args.train_batch_size:
                                                          (real_index + 1) * self.args.train_batch_size]
                except:
                    mask_pos = mask_pos_list['positions'][real_index * self.args.train_batch_size:]
                # Use the mask position to select the relevant activation
                selected_activation = output[torch.arange(output.shape[0]), mask_pos, :]

                # Store the selected activation
                if layer_name in activations:
                    activations[layer_name].append(selected_activation)
                else:
                    activations[layer_name] = [selected_activation]

                # Update the batch counter for the next hook call
                mask_pos_list['batch_counter'] += 1

            return hook

        for name, layer in model.named_modules():
            if name in name_list:
                hook_handles[name] = layer.register_forward_hook(get_activation(pos_mask_list, name))
        ...
        for key in activations.keys():
            temp_mat = activations[key]
            temp_mat = torch.cat(temp_mat)
            temp_mat = self.normlize(temp_mat)
            project = torch.mm(temp_mat,lowrank[key].to("cuda:0"))
            manifold_loss += torch.norm(temp_mat-project)


        loss += lamb*manifold_loss
        if not return_vector_loss:
            loss = loss.mean(dim=0)
    ...
    def training_step(self, model: nn.Module, inputs: Dict[str, Union[torch.Tensor, Any]]) -> dict:
        ...
        scalar_loss = scalar_loss.detach()
        return dict(vector_loss=vector_loss, scalar_loss=scalar_loss)
    def training_step_reg(self, model: nn.Module,emb: torch.Tensor,lamb:float, inputs: Dict[str, Union[torch.Tensor, Any]]) -> dict:
        model.train()
        inputs = self._prepare_inputs(inputs)
        loss = self.compute_loss_reg(model, emb,lamb,inputs, return_vector_loss=True)  # (batch_size,).
        vector_loss = loss
        scalar_loss = loss.mean(dim=0) / self.args.gradient_accumulation_steps
    ...
    def training_step_lowrank(self, model: nn.Module,lowrank: dict,pos_mask_list:dict,lamb:float, inputs: Dict[str, Union[torch.Tensor, Any]]) -> dict:
        model.train()
        inputs = self._prepare_inputs(inputs)
        loss = self.compute_loss_lowrank(model, lowrank,pos_mask_list,lamb,inputs, return_vector_loss=True)  # (batch_size,).
        vector_loss = loss
        scalar_loss = loss.mean(dim=0) / self.args.gradient_accumulation_steps
    ...
    def evaluate_and_log(
        ...
        if objective > self.objective:
            logger.info("Best dev result: {}".format(objective))
            self.objective = objective
            self.model.save_pretrained("prefix_model")
            print("save id")
    ...
    def normlize(self,a):
        ...
        mean_correction = torch.mm(torch.mm(ones, ones.t()), a) / sc
        return a - mean_correction

    def SVD(self,A, THD=0.99):
...
class Trainer_awp(transformers.Trainer):
    """
    Adding some functions based on Transformers' Trainer class.
    """

    def __init__(self, name=None,model_args=None, privacy_args=None, auxiliary_args=None, awp=None,**kwargs):
        super(Trainer_awp, self).__init__(**kwargs)
        self.privacy_args = privacy_args
        self.model_args = model_args
        self.auxiliary_args = auxiliary_args
        self.scaler = torch.cuda.amp.GradScaler(init_scale=128)
    ...
    def train(self,emb,lamb, model_path=None, dev_objective=None, dev_objective_key=None):
        """
        Main training entry point.
        ...
        for epoch in train_iterator:
            ...
            for step, inputs in enumerate(epoch_iterator):
                ...
                if steps_trained_in_current_epoch > 0:
                    steps_trained_in_current_epoch -= 1
                    continue
                if epoch < 15 and (step+1) % self.args.gradient_accumulation_steps == 1:
                    losses = self.training_step(model,emb,lamb, inputs,)
                    tr_loss += losses["scalar_loss"]
                    perb = losses["perb"]
                else:
                    losses = self.training_step_pure(model, emb,lamb,inputs,)
                    tr_loss += losses["scalar_loss"]
                ...
                if (step + 1) % self.args.gradient_accumulation_steps == 0 or (
                    ...
                    self.global_step += 1
                    self.epoch = epoch + (step + 1) / len(epoch_iterator)
                    # restore the weight perb
                    if epoch < 15:
                        self.awp.restore(perb)
                    metrics = None
                    if (
                        self.args.evaluation_strategy in (IntervalStrategy.STEPS, EvaluationStrategy.STEPS) and
                        self.global_step % self.args.eval_steps == 0
    ...
    def training_step(self, model: nn.Module,emb: torch.Tensor,lamb:float, inputs: Dict[str, Union[torch.Tensor, Any]]) -> dict:
        model.train()
        inputs = self._prepare_inputs(inputs)
        perb = self.awp.calc_awp(inputs)
        self.awp.perturb(perb)
        ...
        scalar_loss = scalar_loss.detach()
        return dict(vector_loss=vector_loss, scalar_loss=scalar_loss,perb=perb)

    def training_step_pure(self, model: nn.Module,emb: torch.Tensor,lamb:float, inputs: Dict[str, Union[torch.Tensor, Any]]) -> dict:
        model.train()
        inputs = self._prepare_inputs(inputs)
